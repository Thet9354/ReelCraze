...
//  ReelCraze
//
//  Created by Phoon Thet Pine on 10/12/23.
//
...
class MovieDetailsViewModel: ObservableObject {
    ...
    func loadCastProfiles() async {
        do {
            for member in cast {
                let url = URL(string: "https://api.themoviedb.org/3/movie/\(member.id)?api_key=\(MovieDiscoverViewModel.apiKey)&language=en=US")!
                let (data, _) = try await URLSession.shared.data(from: url)
